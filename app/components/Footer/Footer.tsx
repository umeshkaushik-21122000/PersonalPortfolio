...
const ClickableIcon = (props:any) => {
  return (
    <a href={props.href} className="" target={"_blank"} rel="noreferrer">
      <props.Icon className={"w-5 h-5 text-gray-400 hover:text-red-700 fill-current hover:cursor-pointer"} />
    </a>
  );
};
const IconsData = [
  { href: "https://github.com/umeshkaushik-21122000", Icon: GithubIcon },
  { href: "https://www.linkedin.com/in/umesh-kaushik21/", Icon: LinkedinIcon },
];
...
export default function Fotter(props: { githubUrl: string; hideSocialsInDesktop: boolean }) {
  return (
    ...
    text-sm  space-y-2  "
        >
          <span className="group-hover:text-red-700 sm:text-sm text-xs">
            Built by Umesh Kaushik
          </span>

          <span className="text-xs flex flex-row items-center space-x-2 group-hover:text-red-700">
            <GithubIcon className={"w-4 h-4 text-gray-400 fill-current group-hover:text-red-700"} />
